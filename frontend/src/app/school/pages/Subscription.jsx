...
const Subscription = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // 1. Fetch available plans for school
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        ...
        <span className="ml-3 font-semibold text-slate-600">Loading subscription billing portal...</span>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto text-left pb-12 space-y-6">
      {/* Page Header */}
      <div className="mb-5 mt-2">
        ...
        <p className="text-[#64748B] text-[15px] font-medium mt-1">
          Manage your School CRM subscription tier, review request logs, and upgrade features.
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            <div className="bg-slate-50/50 border border-[#E8ECF3] p-4 rounded-2xl space-y-2">
              ...
              <div>
                ...
                <p className="font-extrabold text-slate-800 capitalize mt-0.5">{plan.replace('-', ' ')}</p>
              </div>
            </div>
            ...
            <div className="bg-slate-50/50 border border-[#E8ECF3] p-4 rounded-2xl space-y-2">
              ...
              <div>
                <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block">Plan Status</span>
                <p className="font-extrabold text-slate-850 capitalize mt-0.5">{status === 'active' ? 'Active' : 'Suspended'}</p>
              </div>
            </div>
            ...
            <div className="bg-slate-50/50 border border-[#E8ECF3] p-4 rounded-2xl space-y-2">
              ...
              <div>
                <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block">Assessments</span>
                <p className="font-extrabold text-slate-800 mt-0.5">
                  {currentSub?.assessmentEnabled ? '✓ Enabled' : '❌ Disabled'}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="space-y-4">
        <h3 className="text-xs font-extrabold text-slate-450 uppercase tracking-wider text-center block pt-4">
          Choose a Plan for School CRM
        </h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {plans.map((p) => {
            const isCurrent = plan === p.planCode && isSubscriptionActive;
            const isRecommended = p.planCode === 'school-premium';
            const isButtonDisabled = isCurrent || pendingRequest;

            return (
              <div 
                key={p._id} 
                className={`bg-white border rounded-[24px] overflow-hidden flex flex-col justify-between shadow-[0_10px_28px_rgba(15,23,42,0.05)] hover:-translate-y-1 transition-all duration-200 ${
                  isRecommended ? 'border-[#E91E63] ring-1 ring-[#E91E63]/30' : 'border-slate-100'
                }`}
              >
                {/* Header Gradient */}
                <div className={`p-6 text-white text-left relative ${
                  isRecommended ? 'bg-gradient-to-r from-[#E91E63] to-[#F43F7A]' : 'bg-gradient-to-r from-slate-700 to-slate-900'
                }`}>
                  {isRecommended && (
                    <span className="absolute top-4 right-4 bg-white/20 backdrop-blur-md text-white font-extrabold text-[9px] uppercase tracking-wider px-2.5 py-0.5 rounded-full">
                      Recommended
                    </span>
                  )}
                  <span className="text-[9px] font-bold uppercase tracking-widest block bg-white/25 w-max px-2 py-0.5 rounded-full mb-1">
                    {p.planCode === 'school-premium' ? 'Premium Tier' : 'Basic Tier'}
                  </span>
                ...
                <div className="p-6 space-y-6 flex-1 text-left">
                  <div className="space-y-3">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Features & Modules</span>
                    <div className="space-y-2.5">
                      ...
                      {/* Show disabled items for basic */}
                      {p.planCode === 'school-basic' && (
                        <div className="flex items-center space-x-2 text-xs font-semibold text-slate-400">
                          <X className="h-4 w-4 text-rose-455 shrink-0" />
                  ...
                  <Button
                    ...
                    className={`w-full py-3 text-xs font-extrabold tracking-wide uppercase rounded-xl transition-all ${
                      isCurrent 
                        ...
                        : isRecommended 
                          ? 'bg-[#E91E63] hover:bg-[#D81B60] text-white shadow-md' 
